...
public class ElementalWeaponComponent {
    ...
    public static final String RESONANCE_REACTION_KEY = "resonance_reaction";
    ...
    public static ItemStack withElementAndAccum(ItemStack stack, ElementType type, float accumPoints, float critChance, float critDamage) {
        return withElementAndAccum(stack, type, accumPoints, critChance, critDamage, false);
    }

    public static ItemStack withElementAndAccum(ItemStack stack, ElementType type, float accumPoints, float critChance, float critDamage, boolean resonanceReaction) {
        ...
        customData.update(tag -> {
            ...
            tag.putBoolean(RESONANCE_REACTION_KEY, resonanceReaction);
        });
    ...
    public static boolean hasResonanceReaction(ItemStack stack) {
        if (stack == null || stack.isEmpty()) return false;

        CustomData customData = stack.get(DataComponents.CUSTOM_DATA);
        if (customData == null) return false;

        return customData.copyTag().getBoolean(RESONANCE_REACTION_KEY);
    }
    ...
    public static ItemStack removeElement(ItemStack stack) {
        ...
        if (customData != null) {
            customData.update(tag -> {
                ...
                tag.remove(RESONANCE_REACTION_KEY);
            });
            stack.set(DataComponents.CUSTOM_DATA, customData);
        }
